...
class dfd_recent_comments extends SB_WP_Widget {
	...
	public function __construct() {
		$this->widget_params = array(
			'description' => esc_html__('Advanced recent comments widget.', 'dfd')
		);
	...
	function widget($args, $instance) {
		...
		if ($comments) {
			...
			foreach ((array) $comments as $comment) {
				...
				$d = "F j, Y";
				$output .= '<li class="recentcomments">' . /* translators: comments widget: 1: comment author, 2: post link */ sprintf(_x('%1$s on %2$s', 'widgets', 'dfd'), '<p class="comment-text">' . $comment_text . '</p><p class="comment-text-meta"><span class="author">' . get_comment_author_link() . '</span>', '<a href="' . esc_url(get_comment_link($comment->comment_ID)) . '" class="dfd-post-link" title="'.esc_attr(get_the_title($comment->comment_post_ID)).'" target="_blank">' . get_the_title($comment->comment_post_ID) . '</a></p>' . '<p class="entry-meta"><span class="comments-date">' . get_comment_date($d, $comment->comment_ID)) . '</span></p></li>';
			}
		}
