...
class cmb_Meta_Box_types {
	...
	public function list_input_double( $args = array(), $i, $image = '' ) {
		$args = $this->parse_args( $args, 'list_input', array(
			'type'  => 'radio',
			'class' => 'cmb_option',
			'name'  => $this->_name(),
	...
	public function radio_inline_double() {
		return $this->radio(
			array(
				'options' => $this->concat_options( array( 'label' => 'test' ), 'list_input_double' ),
			),
			'radio_image '
		);
	}
	...
	public function checkbox() {
		...
		return sprintf( '<div class="dfd-switch-button">%s <label for="%s" data-value="%s"><span class="button-animation %s"><span class="dfd-button-on">'.esc_html__('On','dfd').'</span><span class="dfd-button-off">'.esc_html__('Off','dfd').'</span></span></label></div>%s', $this->input( $args ), $this->_id(), $args['value'], $animation_class, $this->_desc() );
	}
		...
		'<div id="', $this->_id( '_status' ) ,'" class="cmb_media_status">';
			if ( ! empty( $meta_value ) ) {
				...
				if ( $this->is_valid_img_ext( $meta_value ) ) {
					...
					echo '<img src="', $meta_value, '" alt="'.esc_attr__('Image', 'dfd').'" />';
