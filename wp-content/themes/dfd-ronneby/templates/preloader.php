...
if(!class_exists('Dfd_Ronneby_Preloader')) {
	class Dfd_Ronneby_Preloader {
		...
		function init() {
			...
			$preloader_style .= $this->preloader_style();
			$preloader_class .= 'dfd_preloader_'.$preloader_style;
			$preloader_class .= $this->preloader_percent();
			$preloader_css .= $this->preloader_bg();
			...
			?>
			<div id="qLoverlay">';
				<div id="qLbar_wrap" class="<?php echo esc_attr($preloader_class) ?>" style="<?php echo wp_kses_data($preloader_css) ?>">
		...
		function image () {
			...
			echo '<div class="qLbar-img"><img src="'.esc_url($logo).'" alt="'.esc_attr__('Preloader logo', 'dfd').'" /></div>';
		}
		...
		function preloader_style() {
			global $dfd_ronneby;
			$preloader_style = '';
			$preloader_page_style = DfdMetaBoxSettings::get('preloader_style');
			if($preloader_page_style) {
				$preloader_style = esc_attr($preloader_page_style);
			} elseif(isset($dfd_ronneby['preloader_style']) && $dfd_ronneby['preloader_style']) {
				$preloader_style = $dfd_ronneby['preloader_style'];
			}
			return $preloader_style;
		}
		function preloader_percent() {
			global $dfd_ronneby;
			$preloader_class = '';
			$preloader_page_style = DfdMetaBoxSettings::get('preloader_style');
