	...
	switch($video_style) {
		...
		case 'vimeo':
			...
			if($dfd_vimeo_video_id && !empty($dfd_vimeo_video_id)) {
				...
				$video = '<iframe id="'.esc_attr($uniqid).'" '.$data_atts.' src="https://player.vimeo.com/video/'.esc_attr($dfd_vimeo_video_id).'?api=1&amp;portrait=0&amp;rel=0&amp;autoplay=1'.esc_attr($extra_url_prop).'" width="100%" height="100%" frameborder="0" class="dfd-bg-frame" allow="autoplay"></iframe>';
				if($video_type != 'full-screen-video') {
