...
if(preg_match('/(?i)msie [1-9]/',$_SERVER['HTTP_USER_AGENT']) === 0) {
	...
	if (!empty($post) && is_object($post)) {
		...
		$side_area_class = '';
		$side_menu_location = '';
		$header_style = Dfd_Theme_Helpers::getHeaderStyleOption();
		...
		if (strcmp($show_side_area, 'off') !== 0) {
			if (strcmp($side_area_widgetised, 'on') === 0) { ?>
				<section id="side-area" class="side-area-widget <?php echo esc_attr($side_area_class); ?>" style="<?php echo wp_kses_data($side_area_css); ?>">
			...
			<?php } else { ?>
				<section id="side-area" class="<?php echo esc_attr($side_area_class); ?>" style="<?php echo wp_kses_data($side_area_css); ?>">
					<div class="dfd-side-area-mask side-area-controller"></div>
					...
					<div class="side_area_title">
						<?php if (isset($dfd_ronneby['side_area_title']['url']) && $dfd_ronneby['side_area_title']['url']) { ?>
							<a href="<?php echo home_url(); ?>" title="<?php esc_attr_e('Side area', 'dfd') ?>"><img src="<?php echo esc_url($dfd_ronneby['side_area_title']['url']); ?>" alt="side-area-logo" /></a>
						<?php } ?>
					</div>
						...
						<?php if (isset($dfd_ronneby['side_area_copyright']) && $dfd_ronneby['side_area_copyright']) : ?>
							<div class="side-area-subbottom">
								<div class="side_area_copyright">
									<?php echo wp_kses_post($dfd_ronneby['side_area_copyright']) ?>
								</div>
							</div>
						<?php endif; ?>
					</div>
