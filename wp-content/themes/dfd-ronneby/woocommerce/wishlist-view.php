                    ...
                    <tr id="yith-wcwl-row-<?php echo esc_attr($item['prod_id']) ?>" data-row-id="<?php echo esc_attr($item['prod_id']) ?>">
                        ...
                        <?php if( $show_price ) : ?>
                            <td class="product-price">
                                <?php echo ($product->get_price()) ? wp_kses_post($product->get_price_html()) : apply_filters( 'yith_free_text', esc_html__( 'Free!', 'dfd' ) ); ?>
                            </td>
                        <?php endif ?>

                        <?php if( $show_stock_status ) : ?>
                            <td class="product-stock-status">
                                <?php echo ($stock_status == 'out-of-stock') ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'dfd' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'dfd' ) . '</span>'; ?>
                            </td>
                        <?php endif ?>
                        ...
                        <td class="product-add-to-cart">
	                        ...
	                        if( $show_dateadded && isset( $item['dateadded'] ) ):
								echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on : %s', 'dfd' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
	                        endif;
	                        ?>
	                        ...
	                        <select class="change-wishlist selectBox">
		                        <option value=""><?php esc_html_e( 'Move', 'dfd' ) ?></option>
		                        <?php
		                        ...
		                        ?>
			                        <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
				                        ...
				                        if( $wl['wishlist_privacy'] == 1 ){
					                        $wl_privacy = esc_html__( 'Shared', 'dfd' );
				                        }
				                        elseif( $wl['wishlist_privacy'] == 2 ){
					                        $wl_privacy = esc_html__( 'Private', 'dfd' );
				                        }
				                        else{
					                        $wl_privacy = esc_html__( 'Public', 'dfd' );
				                        }
	                        ...
	                        <?php if( $is_user_owner && $repeat_remove_button ): ?>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php esc_attr_e( 'Remove this product', 'dfd' ) ?>"><?php esc_html_e( 'Remove', 'dfd' ) ?></a>
                            <?php endif; ?>
                        </td>
	                <?php endif; ?>
                    </tr>
				...
				<div class="info-wrap-empty">
					...
					<div class="info-content-empty">
						...
						<div class="info-empty">
							<p class="cart-empty-text"><?php esc_html_e( 'No products in the list', 'dfd' ) ?></p>
							<p class="cart-empty-subtext"><?php esc_html_e( 'Check the shop to find great items that you will definitely like', 'dfd' ) ?></p>
							<p class="button-on-page"><a class="wc-backward" href="<?php echo apply_filters( 'woocommerce_return_to_shop_redirect', get_permalink( wc_get_page_id( 'shop' ) ) ); ?>"><?php esc_html_e( 'Return to shop', 'dfd' ) ?></a></p>
						</div>
					</div>
				</div>
	        ...
	        <td colspan="<?php echo esc_attr( $column_count ) ?>">
	            ...
	            <?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
		            <div class="ask-an-estimate-button-container">
	                    ...
	                    <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', esc_html__( 'Ask for an estimate', 'dfd' ) ) ?>
	                </a>
		            </div>
	            <?php endif; ?>
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo esc_attr($ask_estimate_url) ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if( ! is_user_logged_in() ): ?>
				<label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', esc_html__( 'Your email', 'dfd' ) ) ?></label>
