...
<?php if(is_singular('product')){ 
	if( $page_layout == '' ){
		$page_layout = isset($milton_options['shop-single-layout'])?esc_attr( $milton_options['shop-single-layout'] ):'container';
	}

	if( $page_sidebar == '' ){
		$page_sidebar = isset($milton_options['shop-single-sidebar'])?esc_attr( $milton_options['shop-single-sidebar'] ):'no-sidebar';
	} ?>
...
<?php } 
	...
	$shop_navigation = isset($milton_options['shop-navigation']) && esc_attr( $milton_options['shop-navigation'] );
    $shop_navigation_choice = isset($milton_options['shop-navigation-choice']) && esc_attr( $milton_options['shop-navigation-choice'] );

	if( $page_remove_title == '' ){
		$page_remove_title = isset($milton_options['page-remove-title']) && esc_attr( $milton_options['page-remove-title'] );
	}
	if( $page_layout == '' ){
		$page_layout = isset( $milton_options['shop-layout'] )?esc_attr( $milton_options['shop-layout'] ):'container';
	}
	if( $page_sidebar == '' ){
		$page_sidebar = isset($milton_options['shop-sidebar']) && esc_attr( $milton_options['shop-sidebar'] );
	} ?>
